...
function calcularIndicadoresMarketing(lancamentos, vendasVendedores = null) {
  const ind = {
  ...
  // O fechamento de vendas é registrado na aba de desempenho por vendedor.
  // Quando esse total é fornecido, ele prevalece sobre a coluna histórica de Marketing.
  if (typeof vendasVendedores === "number") {
    ind.vendas = vendasVendedores;
  }
...
function filtrarMarketingPorIntervalo(dataInicio, dataFim) {
  ...
  const vendasVendedores = filtrarVendedoresPorIntervalo(dataInicio, dataFim)
    .reduce((total, lancamento) => total + lancamento.vendas, 0);
  const outros = filtrarOutrosPorIntervalo(dataInicio, dataFim)
    .reduce((total, lancamento) => total + lancamento.vendas, 0);
  const indicadores = calcularIndicadoresMarketing(filtrados, vendasVendedores + outros);
  ...
  const vendasVendedoresAnterior = filtrarVendedoresPorIntervalo(periodoAnterior.inicio, periodoAnterior.fim)
    .reduce((total, lancamento) => total + lancamento.vendas, 0);
  const outrosAnterior = filtrarOutrosPorIntervalo(periodoAnterior.inicio, periodoAnterior.fim)
    .reduce((total, lancamento) => total + lancamento.vendas, 0);
  const indicadoresAnterior = calcularIndicadoresMarketing(filtradosAnterior, vendasVendedoresAnterior + outrosAnterior);
