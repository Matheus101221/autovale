...
const cardsIndicadoresFinanceiros = [
  ...
  { key: "percentualMedioRetorno", label: "Porcentagem Média Retorno", format: "percentual", description: "Manual: média das porcentagens da planilha ÷ quantidade de contratos" },
...
function encontrarIndicesColunas(json) {
  ...
  if (json.table && json.table.cols) {
    json.table.cols.forEach((col, idx) => {
      ...
      else if (label.includes("porcentagem") || label.includes("percentual")) mapa.percentualManual = idx;
    });
  }
...
function parsearRespostaGoogleSheets(texto) {
  ...
  linhas.forEach((row) => {
    ...
    const percentualManual = parsearNumero(row.c[idx.percentualManual]);

    registros.push({ dataISO, sdr, contratos, faturamento, retorno, tac, consorcio, servicos, percentualManual });
  });
...
function calcularIndicadoresPeriodo(lancamentos) {
  ...
  let somaPercentualManual = 0;
  let qtdRegistrosPercentual = 0;
  ...
  lancamentos.forEach((l) => {
    ...
    if (l.percentualManual) {
      somaPercentualManual += l.percentualManual;
      qtdRegistrosPercentual += 1;
    }
  });
  ...
  /* Porcentagem Média Retorno (manual): média das porcentagens digitadas
     na planilha (uma por SDR/lançamento), dividida pela quantidade de contratos do período */
  const mediaPercentualManual = qtdRegistrosPercentual > 0 ? somaPercentualManual / qtdRegistrosPercentual : 0;
  ind.percentualMedioRetorno = ind.contratos > 0 ? mediaPercentualManual / ind.contratos : 0;
