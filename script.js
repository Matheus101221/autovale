...
function renderizarVendedores(dataInicio, dataFim, custoPorAprovado = 0) {
  ...
  const vendasOutros = filtrarOutrosPorIntervalo(dataInicio, dataFim)
    .reduce((total, lancamento) => total + lancamento.vendas, 0);
  const totalVendas = vendedores.reduce((total, vendedor) => total + vendedor.vendas, 0) + vendasOutros;
  grid.innerHTML = "";

  vendedores.forEach((vendedor) => grid.appendChild(criarCardVendedor(vendedor, totalVendas)));
  grid.appendChild(criarCardVendedor({
    nome: "Outros",
    aprovados: 0,
    vendas: vendasOutros,
    faturamento: 0,
