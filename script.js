...
function renderizarPodio(sdrs, carregando = false) {
  const podium = document.getElementById("showroom-podium");
  if (!podium) return;
  ...
  const ranking = ordenarRanking(sdrs).slice(0, 3);
  podium.innerHTML = "";
  if (!ranking.length) {
    podium.innerHTML = '<p class="showroom-empty">Aguardando resultados para montar o pódio.</p>';
  } else {
    const ordem = ranking.length === 1 ? [ranking[0]] : ranking.length === 2 ? [ranking[1], ranking[0]] : [ranking[1], ranking[0], ranking[2]];
    ordem.forEach((sdr) => {
      const posicao = ranking.indexOf(sdr) + 1;
      const item = document.createElement("div");
      item.className = `showroom-podium__item showroom-podium__item--${posicao}`;
      item.innerHTML = `<span class="showroom-podium__medalha">${posicao}º</span><span class="showroom-podium__avatar"></span><strong></strong><small>${formatarNumero(sdr.contratos)} contratos</small>`;
      item.querySelector(".showroom-podium__avatar").textContent = iniciaisDoNome(sdr.nome);
      item.querySelector("strong").textContent = sdr.nome;
...
function renderizarBancos(bancos, temDados = true, carregando = false) {
  ...
  bancos.forEach((banco, indice) => {
    ...
    fatia.setAttribute("class", `banco-chart__slice banco-chart__slice--${indice % 6}`);
  ...
  bancos.forEach((banco, indice) => {
    ...
    item.innerHTML = `<span class="banco-chart__marker banco-chart__slice--${indice % 6}"></span><span class="banco-chart__name"></span><strong>${formatarNumero(banco.lancamentos)} · ${formatarPercentual(percentual)}</strong>`;
...
function atualizarDashboardPorData() {
  ...
  renderizarPodio(dados.sdrs, carregando);
