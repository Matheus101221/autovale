...
function criarKpiCard(item, valor, indicadores, indicadoresAnterior) {
  ...
  if (item.comparar) {
    ...
    comparativo.innerHTML = `
      ...
      <span>${formatarPercentual(Math.abs(variacao))} vs período anterior</span>
      <span class="kpi-card__comparativo-abs">(${formatarNumero(valorAnterior)})</span>
    `;
...
function inicializarFiltrosDeData() {
  ...
  // Define as datas mínima e máxima automaticamente (união das duas planilhas)
  // se os campos ainda estiverem vazios
  if (!inputInicio.value || !inputFim.value) {
    const datas = [...todosLancamentos, ...todosLancamentosMarketing]
      .map((l) => l.dataISO)
      .filter(Boolean)
      .sort();
    if (datas.length > 0) {
