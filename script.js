...
function agruparDesempenhoVendedores(lancamentos) {
...
function criarCardVendedor(vendedor, totalVendas) {
  ...
  card.innerHTML = `
    ...
    <div class="vendedor-card__metrics">
      <div><span>Aprovados</span><strong>${formatarNumero(vendedor.aprovados)}</strong></div>
      <div><span>Vendas</span><strong>${formatarNumero(vendedor.vendas)}</strong></div>
      <div><span>Faturamento</span><strong>${formatarMoeda(vendedor.faturamento)}</strong></div>
      <div><span>Ticket médio</span><strong>${formatarMoeda(vendedor.ticketMedio)}</strong></div>
    </div>
...
function atualizarDashboardPorData() {
  ...
  renderizarVendedores(inicioVal, fimVal);
}
